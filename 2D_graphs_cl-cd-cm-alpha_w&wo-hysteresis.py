...
plt.plot(df['Alpha'], df['Cm'], color='gray',linewidth='1', label = 'Cm')
plt.legend
...
plt.plot(da['Alpha'], da['Cm'], color='gray',linewidth='1',label = 'Cm')
plt.legend
q=sns.scatterplot(data=da, x=da['Alpha'], y=da['Cl'], marker ='X', color='seagreen')
q.set(xlabel='Alpha [degrees]', ylabel='Cl/Cm [-]')
plt.savefig('.\Measurement_Data\\twoD\Graphs\\'+str(['ClCm-Alpha_nohysteresis'])+'.svg',dpi=1200)
    #plt.savefig('G:\My Drive\TU Delft\LowSpeedWindTunnelTest\LSWTT-Group16\Measurement_Data\\twoD\Graphs\\'+str(df[i]['Alpha'])+'.svg',format='svg', dpi=1200)
plt.clf()

plt.plot(df['Cl'], df['Cd'], color='red',linewidth='1')
plt.legend
...
plt.plot(da['Cl'], da['Cd'], color='red',linewidth='1')
plt.legend
